...
public class SkuFlowLayout extends ViewGroup {
    ...
    public SkuFlowLayout(Context context, AttributeSet attrs) {
        ...
        TypedArray a = context.getTheme().obtainStyledAttributes(
                attrs, R.styleable.SkuFlowLayout, 0, 0);
        try {
            mFlow = a.getBoolean(R.styleable.SkuFlowLayout_flow, DEFAULT_FLOW);
            try {
                mChildSpacing = a.getInt(R.styleable.SkuFlowLayout_childSpacing, DEFAULT_CHILD_SPACING);
            } catch (NumberFormatException e) {
                mChildSpacing = a.getDimensionPixelSize(R.styleable.SkuFlowLayout_childSpacing, (int) ConvertUtils.dp2px(DEFAULT_CHILD_SPACING));
            }
            try {
                mChildSpacingForLastRow = a.getInt(R.styleable.SkuFlowLayout_childSpacingForLastRow, SPACING_UNDEFINED);
            } catch (NumberFormatException e) {
                mChildSpacingForLastRow = a.getDimensionPixelSize(R.styleable.SkuFlowLayout_childSpacingForLastRow, (int) ConvertUtils.dp2px(DEFAULT_CHILD_SPACING));
            }
            try {
                mRowSpacing = a.getInt(R.styleable.SkuFlowLayout_rowSpacing, 0);
            } catch (NumberFormatException e) {
                mRowSpacing = a.getDimension(R.styleable.SkuFlowLayout_rowSpacing, ConvertUtils.dp2px(DEFAULT_ROW_SPACING));
            }
            mMaxRows = a.getInt(R.styleable.SkuFlowLayout_maxRows, DEFAULT_MAX_ROWS);
            mRtl = a.getBoolean(R.styleable.SkuFlowLayout_rtl, DEFAULT_RTL);
        } finally {
            a.recycle();
        }
    }
