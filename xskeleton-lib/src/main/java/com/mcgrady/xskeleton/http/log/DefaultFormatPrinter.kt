...
import com.mcgrady.xskeleton.utils.HttpLogUtils
...
class DefaultFormatPrinter : FormatPrinter {
    ...
    override fun printJsonResponse(chainMs: Long, isSuccessful: Boolean, code: Int, headers: String, contentType: MediaType?,
        ...
        bodyString = if (RequestInterceptor.Companion.isJson(contentType)) CharacterHandler.Companion.jsonFormat(bodyString) else if (RequestInterceptor.Companion.isXml(contentType)) CharacterHandler.Companion.xmlFormat(bodyString) else bodyString
        ...
        HttpLogUtils.Companion.debugInfo(tag, RESPONSE_UP_LINE)
        ...
        private fun logLines(tag: String, lines: Array<String>, withLineSize: Boolean) {
            for (line in lines) {
                ...
                for (i in 0..lineLength / MAX_LONG_SIZE) {
                    ...
                    HttpLogUtils.Companion.debugInfo(resolveTag(tag), DEFAULT_LINE + line.substring(start, end))
                }
            }
        }
